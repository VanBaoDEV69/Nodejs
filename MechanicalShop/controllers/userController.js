  ...
  try {
    ...
    if (!user) {
      ...
      const transporter = nodemailer.createTransport({
        ...
        auth: {
          user: 'thuanphat.helper@gmail.com', // replace with your Elastic Email username
          pass: 'A7ECE53AF586699D4FDDEDDE84FD0AB511FD', // replace with your Elastic Email password
        },
      });
      ...
      const mailOptions = {
        ...
        text: `Bạn nhận được thư này vì bạn (hoặc người khác) đã yêu cầu đặt lại mật khẩu mới(quên mật khẩu cũ).\n
        Mã xác nhận thay đổi mật khẩu: "${token}"\n
        Lưu ý :\n
        - Mã xác thực chỉ có tác dụng trong 5 phút.\n
        - Nếu bạn không không làm điều này hãy bỏ qua email và mật khẩu của bạn sẽ không thay đổi.\n`
      };
...
exports.getOrderUserById = async (req, res, next) => {
  try {
    const orderId = req.params.id;
    const userId = req.query.id; // Lấy userId từ query parameters
    console.log(userId);
    ...
    res.render('order-detail-user', { req, order, user });
  } catch (err) {
    return next(err);
  }
};